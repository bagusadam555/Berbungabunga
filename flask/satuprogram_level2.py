...
def sistem_identifikasi_losses_dataAIS(tipe_kapal,csv_file):
    # Kapal 
    ...
    for i in range(len(kapal)-1):
        ...
        if int(difference.total_seconds()) >= 7200:
            ...
            losses_data = True
        elif 360 < difference.total_seconds() < 7200:
    ...
    if losses_data:
        print(f'Kesimpulan : Pada kapal {tipe_kapal} ditemukan bahwa TELAH TERJADI losses data AIS selama lebih dari 2 jam')
        return 2
    else:
        print(f'Kesimpulan : Pada kapal {tipe_kapal} ditemukan bahwa TIDAK TERJADI losses data AIS selama lebih dari 2 jam')
        return 3